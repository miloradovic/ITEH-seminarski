...
Flight::register('db', 'Database', array('reg_vozila'));
...
Flight::route('POST /tablice', function () {
    ...
    if ($podaci == null) {
        ...
        if (!property_exists($podaci, 'idVlasnik')||!property_exists($podaci, 'idVozilo')||!property_exists($podaci, 'tablica')) {
            $odgovor["poruka"] = "Niste prosledili korektne podatke";
            $json_odgovor = json_encode($odgovor, JSON_UNESCAPED_UNICODE);
            echo $json_odgovor;
            return false;
        } else {
            if ($db->insertTablice($podaci->idVlasnik, $podaci->idVozilo, $podaci->tablica, 'NOW()')) {
...
Flight::route('POST /novosti', function () {
    ...
    if ($podaci == null) {
        ...
        if (!property_exists($podaci, 'naslov')||
            !property_exists($podaci, 'tekst')||
            !property_exists($podaci, 'kategorija_id')) {
            $odgovor["poruka"] = "Niste prosledili korektne podatke";
            $json_odgovor = json_encode($odgovor, JSON_UNESCAPED_UNICODE);
            echo $json_odgovor;
            return false;
